...
Per default, the Neo4j Advanced Server and Neo4j Enterprise Server editions do not allow remote JMX connections, since the relevant options in the 'conf/neo4j-wrapper.conf' configuration file are commented out.
...
On Windows, follow the tutorial at http://docs.oracle.com/javase/6/docs/technotes/guides/management/security-windows.html to set the correct permissions.
...
For opening JMX to remote monitoring access, please see <<jmx-remote>> and http://docs.oracle.com/javase/6/docs/technotes/guides/management/agent.html#gdenl[the JMX documention].
...
include::{importdir}/ha/ops/jmx.asciidoc[]
